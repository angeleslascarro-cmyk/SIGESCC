...
const props = defineProps({ neighbor: Object });

const form = useForm({
  full_name: props.neighbor.full_name,
  cedula: props.neighbor.cedula,
  phone: props.neighbor.phone || '',
  address: props.neighbor.address || '',
  credit_limit_bs: props.neighbor.credit_limit_bs ?? 0,
  credit_limit_usd: props.neighbor.credit_limit_usd ?? 0,
});

function submit() {
  form.put(route('neighbors.update', props.neighbor.id));
}
</script>
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mb-4 flex items-center justify-between">
      <h1 class="text-xl font-semibold">Editar Vecino</h1>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            step="0.01"
            min="0"
            v-model.number="form.credit_limit_bs"
            class="w-full border rounded p-2"
          />
          <div v-if="form.errors.credit_limit_bs" class="text-red-600 text-sm">
            {{ form.errors.credit_limit_bs }}
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            step="0.01"
            min="0"
            v-model.number="form.credit_limit_usd"
            class="w-full border rounded p-2"
          />
          <div v-if="form.errors.credit_limit_usd" class="text-red-600 text-sm">
            {{ form.errors.credit_limit_usd }}
          </div>
        </div>
      </div>
